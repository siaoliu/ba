...
class Bottleneck(nn.Module):
    ...
    def mask(self,x,soft = False):
        ...
        threshold = mask.view(mask.size(0),-1)
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
class stn(nn.Module):
    def __init__(self,inchannels,width):
        super(stn,self).__init__()
        self.inchannels = inchannels
        self.width = width
        self.location = nn.Sequential(
            nn.Conv2d(inchannels,inchannels,stride=1,kernel_size=1),#conv1x1
            nn.BatchNorm2d(inchannels),
            nn.ReLU(True),
            nn.Conv2d(inchannels, inchannels, stride=1, kernel_size=3,padding=1),  # conv3x1
            nn.BatchNorm2d(inchannels),
            nn.ReLU(True),
            nn.Conv2d(inchannels, inchannels, stride=1, kernel_size=1),  # conv1x1
            nn.BatchNorm2d(inchannels),
        ...
        self.fc_loc = nn.Sequential(
            nn.Linear(inchannels*width*width,64),
            nn.ReLU(True),
            nn.Linear(64,6)
        )
    def forward(self,x):
        xs = self.location(x)
        xs = xs.view(-1, self.inchannels * self.width * self.width)
...
class stn_resnet101(nn.Module):
    def __init__(self):
        super(stn_resnet101,self).__init__()
    def forward(self,x):
        _,a,b,_ = x.size()
        pre = stn(a,b)
