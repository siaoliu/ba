...
class Bottleneck(nn.Module):
    ...
    def __init__(self,in_channels,out_channels,stride=1,downsample=None):
        ...
        self.self.threshold = nn.Sequential(
            conv3x3(out_channels*self.expansion,out_channels*self.expansion),
            nn.ReLU(True),
            conv3x3(out_channels * self.expansion, out_channels * self.expansion),
            nn.Sigmoid()
        )
    ...
    def mask(self,x,soft = False):
        ...
        mask = torch.where(x>threshold,torch.ones_like(x),torch.zeros_like(x))
        return mask
...
class ResNet(nn.Module):
    ...
    def __init__(self,block,layers,num_classes=10):
        ...
        self.avgpool = nn.AvgPool2d(4, stride=1)  # 平均池化，滤波器为7*7，步长为1，特征图大小变为1*1
